// =====================================================================
// Peachtree Roofing — Inbound SMS Logger  (Cloudflare Worker: peachtree-sms-logger)
//
// WHAT THIS DOES
// When someone replies to one of the T1-T8 group texts, this Worker
...
// TC has its own deployment (acculynx-sms-logger) with its own
...
// ---- BRAND CONFIG (Peachtree) ----
const BRAND            = "Peachtree";
const LOG_TIMEZONE     = "America/New_York"; // Peachtree operates on Eastern
const TZ_LABEL         = "ET";
const RC_FROM          = "+14043295117";    // Peachtree's T1-T8 texting number
const GS_SHEET_ID      = "YOUR_PT_GS_SHEET_ID";
const GS_SHEET_NAME    = "Log 2026";
const ACCULYNX_API_KEY = "YOUR_PT_ACCULYNX_API_KEY";
...
// participant, so it must not resolve to a job. Brand-specific — Peachtree's app
// uses +14045768975 (see DEDUP_PHONE in index.template.html), TC's +12145098272.
...
// app cannot list it (GET /subscription is scoped to the authenticating user and
// app) and cannot delete it either — DELETE returns 404 CMN-102. Without a guard,
// each delivery posts its own AccuLynx note.
//
